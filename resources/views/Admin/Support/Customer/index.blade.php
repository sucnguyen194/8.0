...
@section('title') Ý kiến khách hàng @stop
@section('content')
                ...
                <div class="page-title-box">
                    <div class="page-title-right">
                        <ol class="breadcrumb m-0">
                            ...
                            <li class="breadcrumb-item active">Ý kiến khách hàng</li>
                        </ol>
                    </div>
                    <h4 class="page-title">Ý kiến khách hàng</h4>
                </div>
            </div>
        </div>
        <!-- end page title -->
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-3 col-md-4 mb-lg-0 mb-md-0">
                                ...
                                <div class="mb-2 mb-lg-0 mb-md-0">
                                    ...
                                    <a class="btn btn-default waves-effect waves-light" href="{{route('admin.supports.customers.index')}}"><span class="icon-button"><i class="fe-arrow-left"></i></span> Quay lại</a>
                                </div>
                            </div>
                        </div>
                    </form>
        ...
        <div class="row">
            <div class="col-12">
                <div class="card-box table-responsive">
                    <form method="post" action="{{route('admin.supports.delete')}}" enctype="multipart/form-data">
                        @csrf
                        <div class="action-datatable mb-3">
                            ...
                            <a href="{{route('admin.supports.customers.create')}}" class="btn btn-primary waves-effect width-md waves-light float-right">
                        ...
                        <table id="datatable-buttons" class="table table-bordered table-hover bs-table" style="border-collapse: collapse; border-spacing: 0; ;">
                            ...
                            <tr>
                                <th>
                                    <div class="checkbox">
                                        <input id="check-all" class="check_del" {{$supports->count() == 0 ? "disabled" : "" }} type="checkbox" name="checkAll">
                            ...
                            @foreach($supports as $item)
                                <tr>
                                    ...
                                    <td>{{$item->id}}</td>
                                    <td class="position-relative"><input style="width: 120px" type="number" class="form-control" name="sort" data-id="{{$item->id}}" value="{{$item->sort}}"></td>
                                    <td>{{ $item->name}}</td>
                                    <td>
                                    ...
                                    <td>
                                        <a href="{{route('admin.supports.customers.edit',$item)}}" class="btn btn-primary waves-effect waves-light">
                                            <span class="icon-button"><i class="fe-edit-2"></i></span></a>

                                        <a href="{{route('admin.supports.remove',$item->id)}}" onclick="return confirm('Bạn có chắc muốn xóa?');" class="btn btn-warning waves-effect waves-light">
...
<input type="hidden" class="type" value="{{\App\Enums\SystemsModuleType::SUPPORT}}">
@endsection
