            ...
            <div class="col-12">
                <div class="card-box table-responsive">
                    <div class="action-datatable text-right mb-3">
                        <a href="{{route('admin.roles.create')}}" class="btn btn-primary waves-effect width-md waves-light">
                    ...
                    <table id="datatable-buttons" class="table table-bordered table-hover bs-table" >
                        ...
                        @foreach($roles as $role)
                            <tr>
                                <td>{{$role->id}}</td>
                                <td>{{$role->name}}</td>
                                <td>{!! $role->permissions->pluck('title')->map(function($role){ return '<span class="badge badge-primary">'. $role.'</span>';})->join(' ') !!}</td>
                                <td>
                                    <a href="{{route('admin.roles.edit',$role)}}" class="btn btn-primary waves-effect waves-light">
