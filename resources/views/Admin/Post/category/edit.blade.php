                ...
                <div class="col-lg-4">
                    ...
                    <div class="card-box">
                        ...
                        @foreach($langs as $lang)
                            <a href="{{route('admin.posts.categories.lang',[$lang->value,$category->id])}}" class="btn btn-primary waves-effect width-md waves-light mb-1"><span class="icon-button"><i class="fe-plus"></i> {{$lang->name}}</a>
                        @endforeach

                        @if($category->postLangsBefore)
                            @foreach($lists as $item)
                                <a href="{{route('admin.posts.categories.edit',$item->id)}}" class="btn btn-purple waves-effect waves-light mb-1"><span class="icon-button"><i class="fe-edit-2" aria-hidden="true"></i></span> {{$item->language->name}} #{{$item->id}}</a>
                            @endforeach
                        @endif
                    </div>
                </div>
